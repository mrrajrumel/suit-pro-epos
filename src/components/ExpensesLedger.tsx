...
export default function ExpensesLedger({ isIpsHighContrast = false }: { isIpsHighContrast?: boolean }) {
  ...
  const handleLogExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataResult = await res.json().catch(() => ({}));
      if (!res.ok) {
        setErrorNotice(`Expense saved locally, but server synchronization failed: ${dataResult.error || "Server upload failure"}`);
      }
      ...
      setNotice(`Expense successfully recorded locally under ID ${expId}.`);
      await loadExpenses();
      ...
      <div className="space-y-6">
        ...
        <div className={`border rounded-xl p-5 shadow-lg space-y-4 transition-colors ${
          ...
          <form onSubmit={handleLogExpense} className="space-y-4 text-xs">
            ...
            >
              <PlusCircle className="w-4 h-4" /> {isLoading ? "Saving..." : "Log Expense into Ledger"}
            </button>
          </form>
