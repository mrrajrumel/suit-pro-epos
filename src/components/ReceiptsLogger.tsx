...
export default function ReceiptsLogger({ isIpsHighContrast = false }: { isIpsHighContrast?: boolean }) {
  ...
  // Payments summary
  const getTotalsByGroup = (group: "Cash" | "Card" | "Bank & Others") => {
    return receipts.filter(r => {
      if (group === "Cash") return r.method === "Cash";
      if (group === "Bank & Others") {
        return r.method === "Bank Transfer" || r.method === "Gift Card" || r.method === "Store Credit" || r.method === "Custom" || r.method === "Bank & Others";
      }
      return r.method !== "Cash" && r.method !== "Bank Transfer" && r.method !== "Gift Card" && r.method !== "Store Credit" && r.method !== "Custom" && r.method !== "Bank & Others"; // Card payment channels
    }).reduce((s, r) => s + r.amount, 0);
  };

  const cashTotal = getTotalsByGroup("Cash");
  const cardTotal = getTotalsByGroup("Card");
  const bankOtherTotal = getTotalsByGroup("Bank & Others");
          ...
          <div className="space-y-3">
            ...
            <div className={`border p-3.5 rounded-lg flex items-center justify-between transition-colors ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-gray-500">Visa, Mastercard, AMEX, Contactless</div>
                </div>
              </div>
              <div className="text-right">
            ...
            <div className={`border p-3.5 rounded-lg flex items-center justify-between transition-colors ${
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className={`text-xs font-semibold font-sans ${isIpsHighContrast ? "text-neutral-900" : "text-white"}`}>Bank, Gift Cards & Others</span>
                  <div className="text-[10px] text-gray-500">Transfers, Gift cards, Custom rails</div>
                </div>
              </div>
              <div className="text-right">
                <span className={`font-mono text-sm font-bold ${isIpsHighContrast ? "text-neutral-900" : "text-white"}`}>£{bankOtherTotal.toFixed(2)}</span>
                <div className="text-[9px] text-emerald-400 font-mono leading-none">{grandTotal > 0 ? ((bankOtherTotal/grandTotal)*100).toFixed(0) : 0}% share</div>
              </div>
            </div>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-[10px] uppercase font-bold font-mono ${
                          rec.method === "Cash" 
                            ...
                            : rec.method === "Bank Transfer" || rec.method === "Gift Card" || rec.method === "Store Credit" || rec.method === "Custom" || rec.method === "Bank & Others"
